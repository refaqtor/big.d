...
unittest
{
    ...
    assert(application.getName() == "foo",
        "this() takes the application name as its first argument");
    assert(application.getVersion() == "bar",
        "this() takes the application version as its second argument");
    assert(application.all().keys() == ["help", "list"],
        "this() registered the help and list commands by default");
}
...
{
    auto application = new Application("foo", "bar");
    assert(application.getLongVersion() == "foo <info>bar</info>",
...
unittest
{
    ...
    assert(typeid(commandMap["help"]) == typeid(HelpCommand),
...
unittest
{
    ...
    assert(!application.has("afoobar"),
...
unittest
{
    ...
    assert(application.getNamespaces() == ["foo"],
...
unittest
{
    auto application = new Application;
    application.add(new FooCommand);
    assert(application.findNamespace("foo") == "foo",
        ".findNamespace() returns the given namespace if it exists");
    assert(application.findNamespace("f") == "foo",
...
unittest
{
    ...
    assert(typeid(application.find("foo:bar")) == typeid(FooCommand),
        "find() returns a command if its name exists");
    assert(typeid(application.find("h")) == typeid(HelpCommand),
        "find() returns a command if its name exists");
    assert(typeid(application.find("f:bar")) == typeid(FooCommand),
        "find() returns a command if its name exists");
    assert(typeid(application.find("f:b")) == typeid(FooCommand),
        "find() returns a command if its name exists");
    assert(typeid(application.find("a")) == typeid(FooCommand),
        "find() returns a command if its name exists");
}
...
unittest
{
    ...
    assert(typeid(Foo3Command) == typeid(application.find("foo3:bar")),
        "find() returns the good command even if a namespace has same name");
    assert(typeid(Foo4Command) == typeid(application.find("foo3:bar:toh")),
...
unittest
{
    ...
    {
        assert(
            typeid(e) == typeid(CommandNotFoundException),
            "find() throws a CommandNotFoundException if command does not exist, with alternatives");
        assert(
            e.msg.indexOf(
    ...
    try
    {
        ...
        assert(
            false, "find() should throw an CommandNotFoundException if a command is ambiguous because of a subnamespace, with alternatives");
    }
    catch (Exception e)
    {
        assert(
...
unittest
{
    ...
    catch (CommandNotFoundException e)
    {
        ...
        assert(e.msg.indexOf(format("Command '%s' is not defined.",
            commandName)) > -1,
    ...
    catch (CommandNotFoundException e)
    {
        ...
        assert(
            e.msg.indexOf(
            "foo:bar2") == -1,
            "find() throws a CommandNotFoundException if command does not exist, with alternative : 'foo:bar'");
    }
}
...
unittest
{
    ...
    catch (CommandNotFoundException e)
    {
        ...
        assert(
            e.msg.indexOf(
            "There are no commands defined in the 'foo2' namespace.") > -1,
...
unittest
{
    ...
    assert(application.areExceptionsCaught());
    tester.run(["command" : "foo"], ["decorated" : "false"]);
    writeln(tester.getDisplay(true));
